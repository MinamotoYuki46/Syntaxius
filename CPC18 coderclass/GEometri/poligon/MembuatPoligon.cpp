...
int32_t main(){
  ...
  REP(i, n){
    while(k >= 2 && cross_prod(hull[k-2], hull[k-1], pts[i]) <= 0) k--;
    hull[k++] = pts[i];
  }

  int lo = k + 1;
  REPD(i, n - 1){
    while(k >= lo && cross_prod(hull[k-2], hull[k-1], pts[i]) <= 0) k--;
    hull[k++] = pts[i];
  }

  hull.resize(k - 1);

  if ((int) hull.size() == n) cout << "YA";
  else cout << "TIDAK";
